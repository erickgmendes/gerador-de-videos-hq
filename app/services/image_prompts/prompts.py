...
nomes dos personagens mencionados nesta cena, e a descrição visual de
TODOS os personagens já definidos em cenas anteriores deste mesmo
projeto (pode estar vazia, se esta for a primeira cena com personagens).
...
- Para personagens genuinamente novos: crie uma descrição visual
  ...
  natural dentro do mundo bíblico retratado. Escreva em inglês. Dê a ele
  um nome canônico único (evite variações como "Anjo X" numa cena e só
  "X" noutra — prefira sempre o nome mais específico e completo).
- Para CADA painel, escreva uma descrição autossuficiente em inglês que
  ...
  nome) — a ferramenta que vai gerar a imagem não tem acesso a nada além
  do texto deste painel. Nunca escreva "same as before", "as previously
  described" ou qualquer referência externa. Isso vale mesmo repetindo a
  descrição inteira em todo painel em que o personagem aparecer.
- Descreva a ação, posição corporal, expressão, emoção e o cenário
  (arquitetura, iluminação, objetos, época do dia) de acordo com a cena.
  Quando o roteiro não especificar o local exatamente, mantenha
...
- Quando a cena envolver anjos, milagres ou manifestações divinas,
  ...
  grandiosa, mas ainda dentro da linguagem de quadrinho vintage.
...
- NÃO inclua o bloco de estilo artístico, formato, proibição de texto ou
  ...
  SOMENTE a descrição da cena/personagens de cada painel.
...
{"new_characters": {"Nome": "descrição visual completa em inglês"}, "panels": [{"shot_type": "...", "description": "..."}]}

O array "panels" deve conter exatamente a quantidade de painéis pedida,
na ordem em que devem aparecer. "new_characters" deve conter APENAS
personagens genuinamente novos — é um objeto vazio {} sempre que todos
os personagens da cena já corresponderem a alguém já definido."""
...
_STYLE_BLOCK = (
    ...
    "anatomy, heroic proportions, dramatic cinematic compositions, strong black shadows, "
...
_QUALITY_BLOCK = (
    ...
    "work, realistic anatomy, detailed facial features, detailed clothing, detailed environment, "
    "cinematic composition, dramatic lighting, vintage printed comic texture, hand-painted colors."
)
